...
- 配置保存后按影响范围热重载服务
- 提供可选的管理台密码保护
- 提供首次使用向导、系统健康状态、实时日志、严格脱敏诊断包与版本检测
...
2. 按首次使用向导设置管理密码并扫码登录小米账号
3. 让向导自动发现并选择音箱；普通用户无需手填 DID
4. 保持自动网络设置并完成检查；相关服务会自动热重载

只有全新创建的配置会进入向导；从旧版本升级时不会误触发。之后可在「系统设置 → Setup & Recovery」重新运行向导，现有配置不会被清空。
...
| 页面 | 能力 |
|---|---|
| 播放控制 | 查看 System Health、Xiaomi 登录、协议/FFmpeg、音箱在线及播放来源，控制 URL 投放、暂停、停止、音量和进度 |
| 设备管理 | 从小米云设备中选择一个或多个音箱，并查看 Online / Offline / Unknown |
| 系统设置 | 登录凭据、广播地址、端口、恢复、音量、后台安全和版本检测 |
| 日志与诊断 | 查看实时日志并下载包含运行环境、协议和音箱状态的严格脱敏诊断包 |
