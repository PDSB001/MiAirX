...
- 后台密码连续输错 5 次后，同一直接来源地址会被限制 5 分钟；反向代理还应配置自己的限流策略。
- 登录会话最多保留 24 小时；修改后台密码后，所有浏览器都必须使用新密码重新登录。
- 不要把 `conf/` 挂载到多人可读目录。
- Docker/NAS 上建议限制配置目录权限。
- 优先使用管理台生成的基础脱敏诊断包；公开分享前仍应检查账号、DID、媒体 URL 和局域网地址。
- 已存在的 `config.json` 如果不是合法 JSON 或不满足端口、音量等约束，MiAirX 会明确报错并停止启动，不会静默回退到默认配置。修复文件后再重新启动即可。
