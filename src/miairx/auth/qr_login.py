...
class QRCodeLogin:
    ...
    async def poll(self) -> dict:
        """Poll once against the lp long-poll endpoint.

        Returns a dict with ``state`` and ``message``; a ``confirmed`` result
        additionally carries ``cookie`` and ``user_id``. The request blocks
        until the scan state changes or ``POLL_TIMEOUT_SECONDS`` elapses.
        """
    ...
    async def _do_poll(self) -> dict:
        ...
        if user_id and pass_token:
            # Mirror miair-next: the long-poll only yields userId + passToken.
            # The passToken is a "V1:" encrypted blob; miservice exchanges it
            # for a serviceToken lazily on the first mi_request() via its own
            # login(), so we do NOT exchange anything here.
            return {
